...
def train(model, dataloader, optimizer, criterion, model_name, scaler, use_wandb, run, current_step, epoch):
    model.lm.train()
...
def evaluate(model, dataset, optimizer, criterion, model_name, use_wandb, run):
    epoch_loss = 0
    model.lm.eval()
    with torch.no_grad():
        for idx, batch in enumerate(dataloader):
...
def main(
    ...
    if use_wandb:
        ...
        run = wandb.init(
            ...
            config={"learning_rate": config["learning_rate"], "dataset": model_name, "epochs": config["epochs"]},
        )
    ...
    optimizer = AdamW(
        model.lm.parameters(), lr=config["learning_rate"], betas=(0.9, 0.95), weight_decay=0.1
    )
    ...
    for epoch in tqdm(range(config["epochs"])):
        current_step = train(model, dataloader_train, optimizer, criterion, model_name, scaler, use_wandb, run, current_step, epoch)
        valid_loss = evaluate(model, dataloader_eval, optimizer, criterion, model_name, use_wandb, run)
